...
namespace Keyence.HostLink.Tests.Net8.WinForms
{
    public partial class MainForm : Form
    {
        ...
        private async void btnRead_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (result.IsSuccess)
                {
                    ...
                    try { sb.AppendLine($"Int16 Value: {result.ToInt16()}"); } catch { }
                    try { sb.AppendLine($"Boolean Value: {result.ToBoolean()}"); } catch { }
                ...
                {
                    txtResult.Text = $"Read failed: {result.Error?.Message ?? "Unknown error"}";
        ...
        private async void btnReadContinuous_Click(object sender, EventArgs e)
        {
            ...
            if (!int.TryParse(txtReadCount.Text, out int count) || count <= 0)
            {
        ...
        private async void btnBatchRead_Click(object sender, EventArgs e)
        {
            ...
            AppendLog($"[.NET 8] Batch read using Span<T> from {address}");

            try
            {
                var result = await _client.ReadContinuousAsync(address, 10);
                if (result.IsSuccess)
                {
                    txtResult.Text = $"Batch read success using .NET 8 Span optimization.\nRaw: {result.RawResponse}";
                    AppendLog("[.NET 8] Batch read success with Span<T> optimization.");
                }
                else
                {
                    txtResult.Text = $"Batch read failed: {result.Error?.Message}";
                }
            }
            catch (Exception ex)
            {
                txtResult.Text = $"Exception: {ex.Message}";
                AppendLog($"[.NET 8] Batch read exception: {ex.Message}");
            }
            finally
            {
        ...
        private async void btnBatchWrite_Click(object sender, EventArgs e)
        {
            ...
            string address = txtAddress.Text.Trim();
            string valuesText = txtBatchValues.Text.Trim();

            if (string.IsNullOrEmpty(address) || string.IsNullOrEmpty(valuesText))
            {
                MessageBox.Show("Please enter address and values.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            btnBatchWrite.Enabled = false;
            AppendLog($"[.NET 8] Batch write to {address}");
            ...
            try
            {
                ...
                {
                    values[i] = short.Parse(parts[i].Trim());
                }

                await _client.WriteContinuousAsync(address, values);
                txtResult.Text = $"Batch write success: {address} with {values.Length} values";
                AppendLog("[.NET 8] Batch write success.");
            }
            catch (Exception ex)
            {
                txtResult.Text = $"Exception: {ex.Message}";
                AppendLog($"[.NET 8] Batch write exception: {ex.Message}");
            }
            finally
            {
                btnBatchWrite.Enabled = true;
            }
        }
